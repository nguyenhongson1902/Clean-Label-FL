...
def narcissus_gen(dataset_path, lab):
    ...
    if os.path.exists('./checkpoint/surrogate_pretrain_label_0_200.pth'):
        surrogate_model.load_state_dict(torch.load('./checkpoint/surrogate_pretrain_label_0_200.pth'))
    ...
    save_path = './checkpoint/surrogate_pretrain_label_0_' + str(surrogate_epochs) +'.pth'
...
if __name__ == "__main__":
    dataset_path = '../data/'
    lab = 0
    trigger = narcissus_gen(dataset_path = dataset_path, lab = lab)

    save_name = './checkpoint/best_noise_label_0'+'_'+ time.strftime("%m-%d-%H_%M_%S",time.localtime(time.time())) 
    np.save(save_name, trigger)
